...
A ausência, parada, timeout ou exceção interna em um provider de evidências (evidences, p_crimescene, custom, 
one):
- **Não** quebra o recurso p_chopshop;
...
O registro dinâmico de providers customizados (RegisterEvidenceProvider e RegisterProvider em dispatch) preserva isolamento estrito de contexto e de identidade de recurso (GetInvokingResource):
...
- Tentativas de re-registro ou sequestro retornam erro controlado (lready_registered / wrong_context) sem causar falha no servidor ou no resource.
...
- Controla e autentica sessões de remoção (emovalToken, startedAt, expiresAt, minDurationMs);
- Rejeita conclusões antecipadas (	oo_fast), tokens divergentes ou requisições expiradas.
...
O 
etId do FiveM é um identificador volátil de rede sujeito a reciclagem por pools de entidades:
- Um veículo recém-criado que reutilize um 
etId antigo **não** herda rastreadores, sessões de remoção ou alertas pendentes de lifecycles anteriores;
- A validação de ciclo de vida (alidateTrackerLifecycle com verificação de handle de entidade, modelo e statebag local pChopTrackerId) invalida e descarta imediatamente rastreadores órfãos ou entidades recicladas;
...
- **Zero ocorrências** do identificador legado pChopPlateOriginal no caminho de produção.

### 9. VIN É Tri-State
O status forense de raspagem de chassi/VIN segue estritamente um contrato tri-state:
- scratched: Confirmação positiva de registro na tabela p_chop_vin_scratched;
...
O callback de perícia policial veicular (p_chopshop:inspectVehicle) aplica uma barreira estrita em camadas:
...
4. Posse obrigatória de ferramenta pericial (parts_scanner ou orensic_kit via InvCount);
5. 
etId validado como inteiro positivo finito;
...
### A. Inutilização por Remoção de Motor (pChopEngineMissing)
A remoção do bloco de motor no desmanche avançado marca de forma autoritativa o veículo como mecanicamente inoperante (pChopEngineMissing), impedindo a ignição e condução por qualquer jogador.
...
Todas as strings de interface pericial, notificações, relatórios do scanner e mensagens de erro possuem equivalência completa nos 5 idiomas suportados (en, pt, es, r, 	r), sem fallback cru ou textos hardcoded.
