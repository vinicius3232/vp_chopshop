...
function BrokerContracts.IsReady()
    return _ready and (_db ~= nil and _db ~= false)
end
...
function BrokerContracts.Match(contract, entitlement)
    ...
    if cType == 'part_type' then
        -- target_key pode ser a commodity ('body_panel', 'catalytic_converter', 'adv_engine', 'tyre') ou partKey direta
    ...
    elseif cType == 'model' then
        -- target_key é o modelo (ex: 'sultan', 'bison')
        if not provenance or not provenance.model or provenance.model == 0 then
            return false, 'provenance_missing'
        end

        local expectedHash = GetHashKey(targetKey)
        local provModel = tonumber(provenance.model) or 0

        -- Comparação segura unsigned/signed
        if provModel == expectedHash or (provModel < 0 and (provModel + 4294967296) == expectedHash) then
            return true, resolvedCommodity
        end
        return false, 'wrong_part'

    elseif cType == 'class' then
        -- target_key é a classe de veículo (ex: 'sports', 'suvs', 'muscle')
    ...
    elseif cType == 'high_value' then
        -- target_key deve constar na allowlist server-side
...
local function pickPoolItem(poolName, trustLevel)
    ...
    local idx = math.floor(_rng() * #eligible) + 1
    if idx > #eligible then idx = #eligible end
    return eligible[idx]
end
...
function BrokerContracts.EnsureGlobalContracts(now)
    ...
    if cfgContracts.Enable == false then return 0 end
    ...
    if not okCnt or not rowsCnt or not rowsCnt[1] then return 0 end
    ...
    for _ = 1, needed do
        local pTypeIdx = math.floor(_rng() * #poolTypes) + 1
        if pTypeIdx > #poolTypes then pTypeIdx = #poolTypes end
        local pType = poolTypes[pTypeIdx]

        local item = pickPoolItem(pType, 4) -- global pool usa trust full
        if item then
            local qty = math.random(item.minQty or 1, item.maxQty or 3)
...
function BrokerContracts.EnsurePersonalContracts(playerKey, trustLevel, now)
    ...
    for _ = 1, needed do
        ...
        if item then
            local qty = math.random(item.minQty or 1, item.maxQty or 2)
...
function BrokerContracts.ReserveFulfillment(contractId, playerKey, trustLevel, now)
    ...
    -- UPDATE atômico com verificação de remaining > 0, state e expiração
    local sql = [[
        UPDATE `vp_chop_broker_contracts`
        SET `remaining` = `remaining` - 1,
            `state` = IF(`remaining` - 1 = 0, 'COMPLETED', `state`),
            `fulfilled_at` = IF(`remaining` - 1 = 0, FROM_UNIXTIME(?), `fulfilled_at`)
        WHERE `id` = ?
          AND (`for_identifier` IS NULL OR `for_identifier` = ?)
          AND `state` IN ('AVAILABLE', 'ACCEPTED')
          AND `remaining` > 0
          AND `min_trust` <= ?
          AND `expires_at` > FROM_UNIXTIME(?)
    ...
    local ok, res = pcall(function()
        return db.query.await(sql, { curTime, tonumber(contractId), playerKey, tLevel, curTime })
    end)
    if not ok or not res then
        return { ok = false, err = 'db_error' }
    end
    ...
    local affected = (type(res) == 'table' and res.affectedRows) or (type(res) == 'number' and res) or 0
    if affected <= 0 then
        -- Se não afetou nenhuma linha, falhou na condição atômica
        local c = BrokerContracts.Get(contractId)
        if not c then
            return { ok = false, err = 'not_found' }
        end
    ...
    -- Carregar estado pós-decremento
    local updated = BrokerContracts.Get(contractId)
    local isCompleted = updated and (updated.remaining == 0 or updated.state == 'COMPLETED') or false

    return {
        ok        = true,
        completed = isCompleted,
        remaining = updated and updated.remaining or 0,
        contract  = updated,
    }
end
...
function BrokerContracts.CompensateReservation(contractId, count)
    ...
    local sql = [[
        ...
        SET `remaining` = `remaining` + ?,
            `state` = IF(`for_identifier` IS NULL, 'AVAILABLE', 'ACCEPTED'),
            `fulfilled_at` = NULL
        WHERE `id` = ?
    ]]

    local ok, res = pcall(function()
        return db.query.await(sql, { addCount, tonumber(contractId) })
    end)
    if not ok or not res then
